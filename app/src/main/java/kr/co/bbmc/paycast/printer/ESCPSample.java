...
//    	String data = "영수증";
    ...
    public int printMultilingualFont() throws UnsupportedEncodingException, InterruptedException
    {
        int nLineWidth = 384;
        ...
        Logger.d("printMultilingualFont start!!");
...
//            }
            ...
            Logger.w("START PRINTER!! PASS BY 2");
            posPtr.setAsync(false);
        ...
        {
            Logger.e("Error Esc Sample Printer by : " + e.getMessage());
            e.printStackTrace();
        }
    ...
    @SuppressLint("NewApi")
    public int customSample1()throws IOException
    {
//        int nLineWidth = 384;
        int max_char = 39;
        ...
        int sts = -1;
        Logger.w("posPtr - " + posPtr);
        try {
            sts = posPtr.printerCheck(); ////posPtr.printerCheck() function 과 같이 쓰면 error 남.
            posPtr.printNormal(ESC + "|cA" + ESC + "|bC" + ESC + "|2C" + "배달 취소주문서[매장용] " + LF + LF);
        } catch (Exception e) {
            e.printStackTrace();
            Logger.e("Error : " + e.getMessage());
        }
        Logger.e("ESCPSample 1 sts=" + sts);
